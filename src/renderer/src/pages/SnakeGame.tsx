...
function SnakeGame(): JSX.Element {
  ...
  function tick() {
    ...
    const willEat = g.current.food && newHead.x === g.current.food.x && newHead.y === g.current.food.y
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (dir) {
        e.preventDefault()
        if (dir !== OPPOSITE[g.current.direction] && g.current.status === 'running')
          g.current.nextDirection = dir
      }
      if (e.key === ' ') {
        e.preventDefault()
  ...
  return (
    <div className="snake-game-page">
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 24 }}>
        <Space size={40}>
          <Statistic title="当前得分" value={score} prefix={<TrophyOutlined />}
            valueStyle={{ color: '#52c41a', fontWeight: 700 }} />
          <Statistic title="最高纪录" value={highScore} prefix="🏆"
            valueStyle={{ color: '#faad14', fontWeight: 700 }} />
        </Space>
      </div>

      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <Card styles={{ body: { padding: 0, lineHeight: 0 } }}
